...
(function(aniversary){
        ...
        UserInfo.prototype = {
            ...
            sendAjax : function(){
                ...
                kd.freeLogin(function(response, b, c){
                    ...
                    $.ajax({
                        'type': 'GET',
                        'url':  site + '/oauth/user/?m=' + Math.random(),
                        'data':{},
                        'dataType': 'json',
    ...
    var AreaDialog = (function(){
        ...
        AreaDialog.prototype.setPos = function(){
            this.node.css({
                'margin-left' : parseInt(this.node.width()) * -0.5,
                'margin-top' : parseInt(this.node.height()) * -0.5,
                'left' : '50%',
    ...
    var AddressDialog = (function(){
        ...
        AddressDialog.prototype.callback = function (){
            this.suicide();
            $('.ui-record,.reward-toast').find('.js-adress').remove();
            UA.adressSubmit = 1;
        };
    ...
    var AlipayDialog = (function(){
        ...
        AlipayDialog.prototype.callback = function (){
            this.suicide();
            //TODO
            $('.ui-record,.reward-toast').find('.get-gift').remove();
        ...
        LotteryToast.prototype.setList = function(data){
            ...
            for(var i = 0; i < data.item.list.length; i++){
                ...
                if(item.prize_id!=0){
                    ...
                    ret += '<li><div class="item-detail"><div class="item-pic A'+ item.prize_id +'"></div><div class="item-name">'+ item.prize_name+'</div></div><div class="check-box" data-id="'+ item.id +'" data-type="normal" data-sign="'+ item.sign +'" data-name="'+ item.prize_name +'"><i></i></div></li>';
                }              
            }
